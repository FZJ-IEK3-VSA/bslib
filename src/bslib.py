...
def load_parameter(model_name: str = None) -> Dict:
    ...
    try:
        parameter['E_BAT'] = (parameter['E_BAT_usable'] / parameter['eta_BAT'] * 100
                              + parameter['E_BAT_usable']) / 2
    except:
        parameter['E_BAT'] = None

    # Mean stationary deviation of the charging power in W
    try:
        parameter['P_PV2BAT_DEV'] = parameter['P_PV2BAT_DEV_IMPORT'] \
                                    - parameter['P_PV2BAT_DEV_EXPORT']
    except:
    ...
    try:
        parameter['P_BAT2AC_DEV'] = parameter['P_BAT2AC_DEV_EXPORT'] \
                                    - parameter['P_BAT2AC_DEV_IMPORT']
    except:
        parameter['P_BAT2AC_DEV'] = None

    # Time constant for the first-order time delay element in s
    try:
        parameter['t_CONSTANT'] = (parameter['t_SETTLING'] 
                                   - round(parameter['t_DEAD'])) / 3
    except:
    ...
    return transform_dict_to_array(parameter)
