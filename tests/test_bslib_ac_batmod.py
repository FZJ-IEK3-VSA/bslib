...
def test_acbatmod_avoid_overcharging(ac_bat):
    assert ac_bat.avoid_overcharging(p_bs=3600, e_b0=8990.0, dt=1) == 0.0
    assert ac_bat.avoid_overcharging(p_bs=-2 * 3600, e_b0=1.0, dt=1) == -3240.0


def test_acbatmod_adjust_to_stationary_deviations(ac_bat):
    assert ac_bat.adjust_to_stationary_deviations(p_bs=36.0) == 35.85
    assert ac_bat.adjust_to_stationary_deviations(p_bs=-38.0) == -37.42
    assert ac_bat.adjust_to_stationary_deviations(p_bs=-1.0) == 0.0


def test_acbatmod_get_battery_state(ac_bat):
    assert ac_bat.get_battery_state(1.0, 0.0) is bsl.BatteryState.CHARGING
    assert ac_bat.get_battery_state(-1.0, 1.0) is bsl.BatteryState.DISCHARGING
    assert ac_bat.get_battery_state(0.0, 0.0) is None


def test_acbatmod_charge_battery(ac_bat):
    assert ac_bat.charge_battery(p_bs=1500.0) == 1430.458346868159


def test_acbatmod_discharge_battery(ac_bat):
    assert ac_bat.discharge_battery(p_bs=-1500.0) == -1574.7343399810065


def test_acbatmod_standy_mode(ac_bat):
    assert ac_bat.standby_mode(soc=0) == (12.1, 0)
    assert ac_bat.standby_mode(soc=1) == (14.9, -0.1)


def test_acbatmod_change_battery_content(ac_bat):
...
def test_acbatmod_simulate(ac_bat):
